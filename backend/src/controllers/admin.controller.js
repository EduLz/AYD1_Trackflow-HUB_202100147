...
const resolveProfileRequest = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            message: `Solicitud ${accion.toLowerCase()} correctamente`
        });
    } catch (error) {
        return res.status(500).json({
            message: error.message
...
const editUser = async (req, res) => {
    try {
        ...
        if (!correo || !id_estado) {
            return res.status(400).json({
                message: "El correo y el estado son obligatorios"
            });
        }
...
const vetoUser = async (req, res) => {
    ...
    try {
        ...
        const correoUsuario = userRes.recordset.length > 0
            ? userRes.recordset[0].correo
            : null;
