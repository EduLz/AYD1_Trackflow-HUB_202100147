    ...
    <main class="co-content">
      ...
      <div class="filtros">
        <select v-model="filtroRuta" id="filtro-ruta" @change="cargarCalificaciones">
          ...
          <option v-for="ruta in rutasDisponibles" :key="ruta.id_ruta" :value="ruta.id_ruta">
          ...
          <div v-else>
            ...
            <div v-else class="form-respuesta">
              <textarea
                ...
                rows="2"
                maxlength="1000"
                placeholder="Escribe la respuesta en nombre de la empresa..."
              ></textarea>
...
<style scoped>
.co-content { margin-top: 60px; margin-left: 240px; padding: 2rem; background-color: #f8fafc; min-height: calc(100vh - 60px); }
.co-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 1.5rem; }
.co-header h1 { font-size: 1.6rem; font-weight: 700; color: #1e293b; margin-bottom: 0.3rem; }
.co-subtitle { font-size: 0.9rem; color: #64748b; }
.resumen-stars { text-align: right; }
.prom-label { font-size: 0.75rem; color: #64748b; display: block; }
.prom-valor { font-size: 1.4rem; font-weight: 700; color: #1e293b; }
.prom-total { font-size: 0.78rem; color: #64748b; margin-left: 0.3rem; }

.filtros { display: flex; gap: 0.75rem; margin-bottom: 1.25rem; }
.filtros select { border: 1px solid #cbd5e1; border-radius: 6px; padding: 0.45rem 0.75rem; font-size: 0.86rem; background: #fff; }

.lista-calificaciones { display: flex; flex-direction: column; gap: 0.9rem; max-width: 860px; }
.card-cal { background-color: #fff; border: 1px solid #e2e8f0; border-radius: 8px; padding: 1.25rem; }

.cal-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.6rem; }
.cal-cliente { font-weight: 700; font-size: 0.92rem; color: #1e293b; display: block; }
.cal-servicio { font-size: 0.8rem; color: #64748b; }
.cal-derecha { text-align: right; }
.puntuacion { font-size: 1rem; font-weight: 700; color: #2563eb; display: block; }
.cal-fecha { font-size: 0.78rem; color: #64748b; }
.cal-comentario { font-size: 0.88rem; color: #334155; margin-bottom: 0.85rem; line-height: 1.5; }
.cal-sin-com { font-size: 0.85rem; color: #94a3b8; font-style: italic; margin-bottom: 0.85rem; }

.respuesta-box { background-color: #f1f5f9; border-left: 3px solid #16a34a; padding: 0.65rem 0.9rem; border-radius: 0 6px 6px 0; }
.resp-label { font-size: 0.73rem; font-weight: 700; color: #16a34a; text-transform: uppercase; display: block; margin-bottom: 0.2rem; }
.resp-texto { font-size: 0.86rem; color: #334155; margin: 0 0 0.2rem; }
.resp-fecha { font-size: 0.75rem; color: #64748b; }

.form-respuesta textarea { width: 100%; border: 1px solid #cbd5e1; border-radius: 6px; padding: 0.5rem 0.75rem; font-size: 0.86rem; font-family: inherit; resize: vertical; box-sizing: border-box; }
.resp-acciones { display: flex; gap: 0.5rem; margin-top: 0.5rem; }
.btn-primary { background-color: #2563eb; color: #fff; padding: 0.5rem 1.1rem; border: none; border-radius: 6px; font-weight: 600; cursor: pointer; }
.btn-primary:disabled { opacity: 0.6; cursor: not-allowed; }
.btn-secondary { background-color: #fff; color: #1e293b; padding: 0.5rem 1.1rem; border: 1px solid #cbd5e1; border-radius: 6px; font-weight: 600; cursor: pointer; }
.btn-sm { padding: 0.35rem 0.75rem; font-size: 0.8rem; }
.btn-accion { padding: 0.3rem 0.65rem; border-radius: 6px; font-size: 0.8rem; font-weight: 600; cursor: pointer; border: 1px solid transparent; }
.btn-responder { border-color: #16a34a; color: #16a34a; background: #fff; }
.btn-responder:hover { background-color: #16a34a; color: #fff; }
.co-estado { text-align: center; padding: 3rem; color: #64748b; font-size: 0.9rem; }

.co-toast { position: fixed; bottom: 2rem; right: 1.5rem; z-index: 300; padding: 0.85rem 1.4rem; border-radius: 6px; font-size: 0.88rem; font-weight: 600; color: #fff; box-shadow: 0 4px 16px rgba(0,0,0,.15); }
