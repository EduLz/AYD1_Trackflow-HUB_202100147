  ...
  <div>
    ...
    <main class="admin-content">
      <div class="admin-header">
        <div>
          <h1>Servicios de Transporte Más Usados</h1>
          <p class="admin-subtitle">Reporte gráfico de las rutas o servicios de transporte con mayor uso en la plataforma.</p>
        </div>
  ...
  setup() {
    ...
    const chartOptions = ref({
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: 'Servicios de Transporte Más Usados',
          font: { size: 16 }
        }
      },
      scales: {
    ...
    onMounted(async () => {
      try {
        ...
        chartData.value = {
          labels: datos.map(d => d.ruta),
          datasets: [
            {
              label: 'Total de Usos',
              backgroundColor: '#3b82f6',
              data: datos.map(d => Number(d.total_usos))
            }
          ]
        };
      } catch (error) {
    ...
    const descargarPDF = () => {
      ...
      if (canvas) {
        ...
        doc.setFontSize(18);
        doc.text('Reporte: Servicios de Transporte Más Usados', 14, 22);
        ...
        doc.save('Reporte_Grafica_Servicios_Transporte.pdf');
      }
    };
