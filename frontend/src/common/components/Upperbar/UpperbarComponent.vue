<template>
  <header class="upperbar-container">
    ...
    <div class="upperbar-actions">
      <button v-if="rolUsuario === 'CLIENTE'" class="cart-button" @click="$emit('toggle-carrito')">
      ...
      <div class="user-profile">
        <div class="avatar-circle">{{ inicialesUsuario }}</div>
        <div class="user-info">
...
onMounted(() => {
  ...
  nombreUsuario.value = localStorage.getItem('tf_user') || 'Usuario';
});

const inicialesUsuario = computed(() => {
  if (!nombreUsuario.value) return 'US';
